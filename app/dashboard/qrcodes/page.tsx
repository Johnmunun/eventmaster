...
export default function QRCodesPage() {
        ...
        const foldersWithCount = await Promise.all(
          data.folders.map(async (folder: FolderItem) => {
            const qrResponse = await fetch(
              `/api/qrcodes?folderId=${folder.id}`
            );
      ...
      if (data.success) {
        ...
        if (typeFilter) {
          filteredQRCodes = filteredQRCodes.filter(
            (qr: QRCodeItem) => qr.type === typeFilter
          );
        }
        ...
        if (debouncedSearch) {
          ...
          filteredQRCodes = filteredQRCodes.filter((qr: QRCodeItem) => {
            ...
            const eventMatch = qr.event?.name
              .toLowerCase()
              .includes(searchLower);
            const guestMatch = qr.guest
              ? `${qr.guest.firstName} ${qr.guest.lastName}`
                  .toLowerCase()
                  .includes(searchLower)
              : false;
  ...
  */
  ...
  const handleDeleteFolder = async (folderId: string) => {
    if (
      !confirm(
        "Êtes-vous sûr de vouloir supprimer ce dossier ? Les QR codes qu'il contient ne seront pas supprimés."
      )
    ) {
      return;
    }
  ...
  const handleDeleteMultipleQRCodes = async () => {
    ...
    if (
      !confirm(
        `Êtes-vous sûr de vouloir supprimer ${count} QR code(s) ? Cette action est irréversible.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      toast.success("QR Codes supprimés", {
        ...
        } QR code(s) supprimé(s) avec succès`,
      });
  ...
  const hasActiveFilters =
    searchQuery || typeFilter || eventFilter || selectedFolder;
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
            ...
            >
              <CardContent className="p-4 sm:p-5">
                <div className="flex flex-col items-center text-center gap-2 sm:gap-3">
                  <div
                    className={`p-3 sm:p-4 rounded-xl bg-gradient-to-br from-primary/20 to-accent/20 shadow-md ${
                  ...
                  <div className="w-full px-1">
                    <p className="font-semibold text-xs sm:text-sm text-gray-900 dark:text-white break-words">
                      Tous
                    </p>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                      {totalQRCodes} QR code{totalQRCodes > 1 ? "s" : ""}
                    </p>
                  </div>
                </div>
              </CardContent>
              ...
              return (
                ...
                >
                  ...
                  <CardContent className="p-4 sm:p-5 relative z-10">
                    <div className="flex flex-col items-center text-center gap-2 sm:gap-3">
                      <div
                        className={`p-3 sm:p-4 rounded-xl ${colors.bg} border-2 ${colors.border} shadow-md group-hover:scale-110 transition-transform duration-300 relative`}
                      >
                        <div
                          className={`absolute inset-0 ${colors.bg} rounded-xl blur-md opacity-0 group-hover:opacity-50 transition-opacity duration-300`}
                        />
                        <Folder
                          className={`h-6 w-6 sm:h-7 sm:w-7 ${colors.icon} relative z-10 drop-shadow-sm`}
                        />
                      </div>
                  ...
                  <div className="flex flex-col md:flex-row md:items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0 space-y-2">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 flex-wrap">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {qr.type === "URL" && (
                            <Badge
                              variant="outline"
                              className="text-xs px-2 py-0.5 rounded-full border-blue-300 text-blue-700 dark:text-blue-400"
                            >
                              QR statique
                            </Badge>
                          )}
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-sm text-gray-600 dark:text-gray-400">
                        ...
                        {qrData.appearanceConfig?.frameStyle &&
                          qrData.appearanceConfig.frameStyle !== "none" && (
                            <div className="flex items-center gap-1.5">
                              <Frame className="h-3.5 w-3.5" />
                              <span className="truncate">Avec cadre</span>
                            </div>
                          )}
                      </div>
                    </div>
      ...
      {/* Drawers */}
      <CreateQRCodeDrawer open={isCreateOpen} onOpenChange={setIsCreateOpen} />
      <CreateFolderDrawer
